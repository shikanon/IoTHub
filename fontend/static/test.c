...
char g_product_key[IOTX_PRODUCT_KEY_LEN + 1]       = "a1L6W2BQytT";
...
/**
 * @brief 上报属性BatteryLevel到云端
 * @param devid 由调用IOT_Linkkit_Open返回的设备标示符
 * @param value 属性BatteryLevel的值，数据类型int32_t
 * @return 消息id:(>=1), 上报失败: <0
 */
int32_t app_post_property_BatteryLevel(uint32_t devid, int32_t value);

/**
 * @brief 上报属性MagneticState到云端
 * @param devid 由调用IOT_Linkkit_Open返回的设备标示符
 * @param value 属性MagneticState的值，数据类型uint32_t
 * @return 消息id:(>=1), 上报失败: <0
 */
int32_t app_post_property_MagneticState(uint32_t devid, uint32_t value);
...
/**
 * @brief 上报事件AbnormalAlarm到云端
 * @param devid 由调用IOT_Linkkit_Open返回的设备标示符
 * @param value 事件AbnormalAlarm的值，数据类型uint32_t
 * @return 消息id:(>=1), 上报失败: <0
 */
int32_t app_post_event_AbnormalAlarm(uint32_t devid, uint32_t value);
...
int32_t app_post_property_BatteryLevel(uint32_t devid, int32_t value)
{
    ...
    res = HAL_Snprintf(property_payload, sizeof(property_payload), "{\"BatteryLevel\": %d}", value);
    if (res < 0) {
...
int32_t app_post_property_MagneticState(uint32_t devid, uint32_t value)
{
    ...
    res = HAL_Snprintf(property_payload, sizeof(property_payload), "{\"MagneticState\": %d}", value);
    if (res < 0) {
        return STATE_SYS_DEPEND_SNPRINTF;
    }
...
{
    int32_t res = STATE_USER_INPUT_BASE;
    char *event_id = "AbnormalAlarm";
    char event_payload[64] = {0};

    res = HAL_Snprintf(event_payload, sizeof(event_payload), "{\"AlarmType\": %d}", value);
    if (res < 0) {
        return STATE_SYS_DEPEND_SNPRINTF;
    }
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        if ((cnt % 20) == 0) {
            app_post_property_BatteryLevel(EXAMPLE_MASTER_DEVID, 0);
            app_post_property_MagneticState(EXAMPLE_MASTER_DEVID, 0);
            app_post_event_Error(EXAMPLE_MASTER_DEVID, 0);
            app_post_event_AbnormalAlarm(EXAMPLE_MASTER_DEVID, 0);
